...
const ArticleDetail = () => {
  ...
  const getArticle = async () => {
    try {
      const res = await axios.get(url + "article/" + articleId);
      setArticle(res.data.article);
      await getImage(res.data.article.image);
    } catch (error) {
      console.error(error);
      setError(2);
    }
  ...
  const getImage = async (articleImg) => {
    try {
      const res = await getImagesFromDS3(articleImg);
